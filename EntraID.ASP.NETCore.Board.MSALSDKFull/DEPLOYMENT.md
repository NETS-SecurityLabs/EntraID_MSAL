...
이 문서는 EntraID.ASP.NETCore.Board.MSALSDK 프로젝트를 Azure App Service에 배포하는 방법을 설명합니다.
...
```
배포 대상 앱 이름: msal-demo-tobe-<alias>
완료: msal-demo-tobe-<alias> 배포 성공
```

배포된 앱에 접속:
- https://msal-demo-tobe-<alias>.azurewebsites.net

예시:
- 로그인 사용자: user@example.com
- 자동 생성 alias: user
- 최종 App Service 이름: msal-demo-tobe-user
...
[4/4] Azure 리소스 확인 및 생성
  ...
  - App Service Plan 확인: easm-msal-demo-plan
    ...
    배포 대상 앱 이름: msal-demo-tobe-user
  - Web App 확인: msal-demo-tobe-user
    Web App 이미 존재
...
예:
- 로그인 사용자: user@example.com
- 기본 앱 이름: MSAL-DEMO-TOBE
- 최종 앱 이름: msal-demo-tobe-user
...
- **기본 App 이름:** `MSAL-DEMO-TOBE`

이는 [MSDeploy 프로필](Properties/PublishProfiles/MSAL-DEMO-TOBE%20-%20Web%20Deploy.pubxml)에서 읽어옵니다.

실제 생성/사용되는 App Service 이름은 다음 규칙을 따릅니다:
- 기본 App 이름을 소문자/하이픈 형식으로 정규화
- 로그인한 Azure 계정의 UPN alias를 suffix로 추가
- 최종 형식: `msal-demo-tobe-<alias>`
...
# 4. App Service 상태 확인
az webapp show --resource-group easm-msal-demo --name msal-demo-tobe-<alias>
```
